  ...
  void _fetchVendorDetails() {
    WidgetsBinding.instance.addPostFrameCallback((_) async {
      ...
      if (cart.vendorId != null) {
        try {
          final vendor = await vendorService.getVendorById(cart.vendorId!);
          if (mounted) {
            setState(() {
              _vendor = vendor;
            });
  ...
  Future<void> _handleCheckout() async {
    ...
    try {
      final payload = {
        ...
        'deliveryAddress': _orderType == 'DELIVERY' ? address : (_vendor?.address ?? 'Store Pick-up'),
        'orderType': _orderType,
        'notes': _orderType == 'DELIVERY' ? _notesController.text.trim() : 'Store Pick-up',
        'paymentMethod': _paymentMethod,
        if (_orderType == 'DELIVERY') 'deliveryLat': _deliveryLocation.latitude,
        if (_orderType == 'DELIVERY') 'deliveryLng': _deliveryLocation.longitude,
      ...
      body: SingleChildScrollView(
        ...
        child: Column(
          ...
          children: [
            ...
            Text('FULFILLMENT METHOD', style: TextStyle(fontWeight: FontWeight.bold, color: Theme.of(context).colorScheme.onSurface.withValues(alpha: 0.6), letterSpacing: 1.2, fontSize: 12)),
            ...
            NeumorphicCard(
              child: Row(
                children: [
                  ...
                  Expanded(
                    child: ChoiceChip(
                      label: const Center(child: Text('Pick-up')),
            ...
            const SizedBox(height: 24),
            Text(_orderType == 'DELIVERY' ? 'DELIVERY DETAILS' : 'PICKUP DETAILS', style: TextStyle(fontWeight: FontWeight.bold, color: Theme.of(context).colorScheme.onSurface.withValues(alpha: 0.6), letterSpacing: 1.2, fontSize: 12)),
            const SizedBox(height: 8),
            NeumorphicCard(
              child: _orderType == 'DELIVERY'
                  ...
                  : Column(
                      ...
                      children: [
                        ...
                        const SizedBox(height: 8),
                        Text(
                          _vendor?.address ?? 'Loading address...',
                          ...
                          child: ClipRRect(
                            ...
                            child: AbsorbPointer(
                              child: Mapcn(
                                ...
                                points: [LatLng(_vendor?.latitude ?? 14.8214, _vendor?.longitude ?? 120.9565)],
                              ),
                            ),
                          ),
                        ),
